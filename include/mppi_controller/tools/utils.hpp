...
#ifndef MPPI_CONTROLLER__TOOLS__UTILS_HPP_
#define MPPI_CONTROLLER__TOOLS__UTILS_HPP_
 ...
 * @param frame Reference frame to use
 */
inline geometry_msgs::TwistStamped toTwistStamped(float vx, float wz, const std_msgs::Header& header)
{
 ...
 * @param frame Reference frame to use
 */
inline geometry_msgs::TwistStamped toTwistStamped(float vx, float vy, float wz, const std_msgs::Header& header)
{
 ...
 */
inline bool withinPositionGoalTolerance(float pose_tolerance, const geometry_msgs::Pose& robot,
 ...
 * @param data Data to use
 */
inline void setPathCostsIfNotSet(CriticData& data, costmap_2d::Costmap2DROS* costmap_ros)
{
 ...
 */
inline float posePointAngle(const geometry_msgs::Pose& pose, double point_x, double point_y, bool forward_preference)
{
 ...
 * @return Angle between two points
 */
inline float posePointAngle(const geometry_msgs::Pose& pose, double point_x, double point_y, double point_yaw)
{
  float pose_x = pose.position.x;
  float pose_y = pose.position.y;
  float pose_yaw = tf2::getYaw(pose.orientation);
                                ...
                                const models::OptimizerSettings& settings)
{
  ...
  auto applyFilter = [&](const xt::xarray<float>& data) -> float { return xt::sum(data * filter, { 0 }, immediate)(); };

  auto applyFilterOverAxis = [&](xt::xtensor<float, 1>& sequence, const float hist_0, const float hist_1,
                                 const float hist_2, const float hist_3) -> void
  {
    unsigned int idx = 0;
    sequence(idx) = applyFilter({ hist_0, hist_1, hist_2, hist_3, sequence(idx), sequence(idx + 1), sequence(idx + 2),
                                  sequence(idx + 3), sequence(idx + 4) });

    idx++;
    sequence(idx) = applyFilter({ hist_1, hist_2, hist_3, sequence(idx - 1), sequence(idx), sequence(idx + 1),
                                  sequence(idx + 2), sequence(idx + 3), sequence(idx + 4) });

    idx++;
    sequence(idx) = applyFilter({ hist_2, hist_3, sequence(idx - 2), sequence(idx - 1), sequence(idx),
                                  sequence(idx + 1), sequence(idx + 2), sequence(idx + 3), sequence(idx + 4) });

    idx++;
    sequence(idx) = applyFilter({ hist_3, sequence(idx - 3), sequence(idx - 2), sequence(idx - 1), sequence(idx),
                                  sequence(idx + 1), sequence(idx + 2), sequence(idx + 3), sequence(idx + 4) });

    for (idx = 4; idx != num_sequences - 4; idx++)
    {
      sequence(idx) =
          applyFilter({ sequence(idx - 4), sequence(idx - 3), sequence(idx - 2), sequence(idx - 1), sequence(idx),
                        sequence(idx + 1), sequence(idx + 2), sequence(idx + 3), sequence(idx + 4) });
    }
    ...
    idx++;
    sequence(idx) = applyFilter({ sequence(idx - 4), sequence(idx - 3), sequence(idx - 2), sequence(idx - 1),
                                  sequence(idx), sequence(idx), sequence(idx), sequence(idx), sequence(idx) });
  };

  // Filter trajectories
  applyFilterOverAxis(control_sequence.vx, control_history[0].vx, control_history[1].vx, control_history[2].vx,
                      control_history[3].vx);
  applyFilterOverAxis(control_sequence.vy, control_history[0].vy, control_history[1].vy, control_history[2].vy,
                      control_history[3].vy);
  applyFilterOverAxis(control_sequence.wz, control_history[0].wz, control_history[1].wz, control_history[2].wz,
...
/**
 * Find first element in iterator that is greater integrated distance than compare value
 */
template <typename Iter, typename Getter>
inline Iter firstAfterIntegratedDistance(Iter begin, Iter end, Getter getCompareVal)
{
  ...
  {
    dist += mbf_utility::distance(*it, *(it + 1));
...
#endif  // MPPI_CONTROLLER__TOOLS__UTILS_HPP_
